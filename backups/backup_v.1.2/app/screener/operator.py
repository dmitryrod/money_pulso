...
from app.config import get_logger, log_debug_event
...
class Operator:
    ...
    async def _maybe_restart_stale_websocket_parsers(
        ...
        for pair_key in list(self._parsers.keys()):
            ...
            if stale_agg:
                await self._restart_websocket_parser_task(pair_key, "agg_trades")
            if stale_lq:
                await self._restart_websocket_parser_task(pair_key, "liquidations")
