...
- Сессии: `SessionMiddleware` с `secret_key=config.cypher_key`. Поверх него: **GZipMiddleware** (сжатие ответов; `text/event-stream` не сжимается), **ProductionAssetCacheMiddleware** — в `EnvironmentType.PRODUCTION` выставляет долгий **`Cache-Control`** для `/admin/statics/*` и сутки для `/admin_api/ui/timezone.js` (`app/middleware/production_asset_cache.py`).
