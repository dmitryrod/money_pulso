...
4. **Scanner/Test вычисления включаются только при активном SSE subscriber**
   В начале цикла `Consumer._check_filters()` вычисляется `test_enabled = test_stream_is_active()`. Дальше `_symbol_check_pair` вызывает `evaluate_test_mode_snapshot(...)` только если этот флаг истинен. При отсутствии подписчиков `source=test` дополнительный test/scanner payload не строится и `broadcast_test_payload(...)` не вызывается.
   Это делает тестовый канал действительно demand-driven. В production-сценарии без открытой test-ленты система не тратит CPU на вторую ветку оценки по каждому символу, не создаёт лишние payload-объекты в памяти и не делает ненужную fan-out доставку по SSE-очередям. Важная граница: базовая проверка фильтров всё равно выполняется, потому что она нужна для боевых сигналов; экономия достигается именно на дополнительной scanner/test-диагностике. Такой гейт удерживает стоимость observability-функций рядом с нулём, пока UI ими не пользуется, и не ухудшает рабочий путь по event loop и executor без активного оператора в админке.
