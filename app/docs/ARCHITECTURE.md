...
- Сессии: `SessionMiddleware` с `secret_key=config.cypher_key` (**подписанная cookie**, payload сессии у клиента; не in-memory map по `username`). Роль и имя — в `request.session` (`app/admin/auth.py`, `app/admin/roles.py`); параллельные клиенты с одной учётной записью demo имеют **разные** cookie. У всех процессов/реплик должен быть **одинаковый** `CYPHER_KEY`, иначе cookie с одного инстанса не принимается другим. Поверх него: **GZipMiddleware** (сжатие ответов; `text/event-stream` не сжимается), **ProductionAssetCacheMiddleware** (ASGI, не `BaseHTTPMiddleware`) — в `EnvironmentType.PRODUCTION` выставляет долгий **`Cache-Control`** для `/admin/statics/*` и сутки для `/admin_api/ui/timezone.js`; реализация без обёртки `BaseHTTPMiddleware`, чтобы SSE (`/admin_api/signals/stream`) не давал ``RuntimeError: No response returned`` (`app/middleware/production_asset_cache.py`).
...
- **CoinMarketCap** (`utils/coinmarketcap_rank.py`): опционально, по API-ключу из окружения; несколько ключей можно перечислить в **`CMC_PRO_API_KEY`** / **`X-CMC_PRO_API_KEY`** через **`|`**, на каждый запрос listings/latest случайный ключ (`secrets.choice`). Без ключей ранги не обновляются (warning в логах). Кеш запускается при старте приложения (**`lifespan`** в `__main__.py`), не при импорте модуля; фон обновляет ранги с интервалом из env (**по умолчанию ~90 мин** + небольшой джиттер). При HTTP **429** — ретраи с **Retry-After** или backoff; кеш при ошибке сохранённых данных не очищается.
