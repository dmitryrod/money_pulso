...
**Ось `yPrice` (цена USDT):** границы **не** оставляем полностью на авто-диапазон Chart.js по сырым min/max (иначе редкие далёкие бары сжимают типичные столбцы по вертикали). В JS считаются `min`/`max` по массиву min/max за минуту: по умолчанию перцентили **p1** по низам и **p99** по верхам (чекбокс **«Срез выбросов (цена)»**), иначе сырые min/max; затем **padding** ~5% к полосе (и не меньше ~0.01% от средней цены, чтобы при плоском ряду ось не схлопывалась). Без валидных баров — снова авто.
...
- Checkboxes: подписи `.stat-toolbar label`, `font-size: 12px`, `color: #8899bb` (в т.ч. **«Срез выбросов (цена)»** — `id="stat-price-clip"`, по умолчанию включён).
