...
class Websocket:
    ...
    async def restart(self) -> None:
        ...
        Если предыдущая попытка подключения упала с 403 Too Many Requests (CloudFront),
        стоит подождать подольше перед новым коннектом. Этим управляет _reconnect_timeout,
        который может быть увеличен вызывающим кодом.
        """
        await self.stop()
    ...
    async def _connect(self) -> None:
        ...
        while self._running:
            try:
                async for conn in websockets.connect(
                    uri=self._url, **self._generate_ws_kwargs()
                ):
                    try:
                        self._logger.info(
                            f"Websocket connection was established to {self._url}"
                        )
                        ...
                        self._logger.error(
                            f"Websocket connection was closed unexpectedly: {e}"
                        )
                    except Exception as e:
                        self._logger.error(
                            f"Unexpected error in websosocket connection: {e}"
                        )
                    finally:
                ...
                body = getattr(response, "body", b"") if response else b""
                text = (
                ...
                if status == 403 and "too many requests" in text.lower():
                    self._logger.error(
                        "Websocket rejected with 403 Too Many Requests. "
                        "Backing off for 300 seconds before reconnect."
                    )
                ...
                self._logger.error(f"Unexpected error while connecting websocket: {e}")
                await asyncio.sleep(self._reconnect_timeout)
                if not self._running:
                    return
    ...
    async def _handle_message(self, message: str | bytes, conn: ClientConnection) -> None:
        ...
        try:
            ...
            # Если очередь переполнена, агрессивно чистим часть старых сообщений,
            # чтобы не душить процессор и не держать гигантский backlog.
            cleaned = 0
            try:
                # Сбрасываем до 500 старых сообщений (или меньше, если их меньше в очереди).
                for _ in range(500):
                ...
                pass
            self._logger.error(f"Message queue is overflow, cleaned {cleaned} messages")
