...
**Как вызывать субагентов:** для каждого шага вызывай инструмент **mcp_task** с параметрами subagent_type (`worker` | `test-runner` | `debugger` | `reviewer-senior` | `documenter`), prompt, description. Не выполняй роли worker/test-runner/reviewer-senior/documenter самостоятельно — только через mcp_task.
...
1. **Worker — реализация**
   - Вызови mcp_task(subagent_type="worker", prompt="...", description="Worker implementation") с задачей из запроса пользователя.
