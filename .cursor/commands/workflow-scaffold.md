...
**Как вызывать субагентов:** для каждого шага вызывай инструмент **mcp_task** с параметрами subagent_type (`worker` | `test-runner` | `debugger` | `documenter`), prompt, description. Не выполняй роли worker/test-runner/documenter самостоятельно — только через mcp_task.
