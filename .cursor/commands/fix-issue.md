...
4. **Test-Runner — верификация**
   Вызови mcp_task(subagent_type="test-runner", prompt="Прогон тестов после исправления issue", description="Test-runner verification") для прогона тестов. Если тесты падают — test-runner исправляет или делегирует debugger. Продолжай к PR только при успешных тестах или явном решении пользователя.
