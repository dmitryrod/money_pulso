---
name: orchestration
description: Полная оркестрация для /workflow-feature. Use when coordinating complex features: planner decomposes, designer specs when needed, worker/refactor implement, test-runner and debugger verify, reviewer-senior checks quality and architecture, security-auditor audits, documenter reports.
---
...
1. **Planner** — декомпозиция на подзадачи с ID, порядком, зависимостями, рекомендуемым субагентом (**designer** / worker / refactor)
2. **Для каждой задачи:** designer (если подзадача — дизайн/спеки) **или** worker/refactor → для кода: test-runner → debugger при падении; для чистых designer-подзадач test-runner пропускается
